<template>
  <div class="goals-block">
    <h3 class="goals-block__title">Цели</h3>
    <el-table
      class="goals-table"
      :data="goals"
      border
    >
      <el-table-column label="Цели" align="left" header-align="center">
        <template #default="{ row }">
          <span class="goals-table__goal">{{ row.title }}</span>
        </template>
      </el-table-column>

      <el-table-column label="Результат" align="center" header-align="center">
        <template #default="{ row }">
          <span class="goals-table__result">
            {{ row.current }} / {{ row.target }} 
            <el-link class="goals-table__percent-link" :underline="false">
